# Полный тест Contractility на целевой машине

Эта инструкция рассчитана на закрытый контур без выполнения `npm install`.
Проект переносится через Git, а команды запускаются из корня клонированного
репозитория Contractility.

Обозначения в примерах:

- `/opt/contractility` — пример каталога приложения на целевой машине
  (замените его на разрешённый каталог, доступный текущему пользователю);
- `/secure/incoming` — каталог с договором, подписанными дополнительными
  соглашениями, новой редакцией DOCX и экспортом из браузера;
- `RUN_ID` — идентификатор запуска, показанный в интерфейсе.

Не используйте тестовые документы с персональными или коммерческими данными,
если передача их содержимого настроенным моделям GigaCode не разрешена
политикой организации.

## 1. Клонирование репозитория на целевой машине

```bash
git --version
mkdir -p /opt
cd /opt
git clone --branch master --single-branch \
  git@github.com:RomanovAV/contractility.git
cd /opt/contractility
```

Если для целевой машины настроен HTTPS-доступ вместо SSH:

```bash
git clone --branch master --single-branch \
  https://github.com/RomanovAV/contractility.git
cd contractility
```

Зафиксируйте точную версию кода, на которой выполняется тест:

```bash
git status --short
git branch --show-current
git rev-parse HEAD
git log -1 --date=iso --format='%H%n%ad%n%an%n%s'
```

Перед началом теста `git status --short` не должен показывать локальных
изменений. Для воспроизводимого приёмочного прогона используйте заранее
согласованный commit SHA или подписанный тег, а не произвольное текущее
состояние ветки.

## 2. Проверка системных зависимостей

```bash
node --version
gigacode --version
zip -v
unzip -v
```

Требуется Node.js 22 или новее. GigaCode CLI должен быть уже установлен,
настроен и авторизован в разрешённом модельном контуре.

## 3. Настройка моделей

```bash
cp config/target.example.json config/target.json
```

Откройте `config/target.json` и замените все значения `MODEL_*` реальными
идентификаторами моделей GigaCode:

- `models.producer` — формирование первого кандидата;
- `models.synthesizer` — арбитраж замечаний и внесение подтверждённых правок;
- пять `models.reviewers[].model` — независимое ревью.

Все роли могут использовать одну модель. Для полноценного межмодельного теста
при наличии нескольких доступных моделей рекомендуется назначить разные модели
рецензентам, producer и synthesizer, но это не является обязательным.
Значение `"default"` означает модель по умолчанию из настроек GigaCode CLI:
Contractility в этом случае не передаёт аргумент `--model`.
Ответ `404 Model not found` для явно заданной модели также автоматически
переключает конкретный запрос на модель CLI по умолчанию.

Проверьте системные команды и выполните короткий запрос к каждой уникальной
модели:

```bash
npm run doctor:target -- --config config/target.json --smoke
```

Ожидаемый результат: все строки начинаются с `✓`, включая каждую модель.

## 4. Автоматические тесты и запуск локального UI

```bash
npm test
node scripts/doctor.mjs
npm start
```

Откройте в браузере:

```text
http://127.0.0.1:4317
```

В интерфейсе:

1. выберите исходный подписанный договор PDF;
2. добавьте все подписанные дополнительные соглашения PDF в хронологическом
   порядке;
   один PDF может содержать несколько самостоятельных соглашений, в том числе
   к разным договорам, если в OCR читаются номер и дата базового договора
   каждого соглашения;
3. дождитесь завершения OCR всех страниц и исправьте существенные ошибки;
4. нажмите «Сформировать мастер-договор» без загрузки драфта;
5. дождитесь статуса «Мастер-договор готов к проверке», проверьте текст и
   историю применённых, исключённых и неподтверждённых соглашений;
6. введите ФИО и нажмите «Подтвердить мастер-договор»;
7. скачайте файл проекта `*.master-contract.json` и текстовую версию TXT;
8. для проверки переносимости сбросьте комплект и загрузите сохранённый
   мастер-договор; убедитесь, что текст и отметка проверки восстановились;
9. загрузите предлагаемый драфт DOCX и нажмите «Сформировать рекомендованное ДС».
   Повторная загрузка исходных PDF и реконструкция не требуются.

С этого момента исходные документы блокируются от изменения. UI сам:

- передаёт файлы только локальному серверу `127.0.0.1`;
- повторно сверяет SHA-256 и создаёт case;
- использует подтверждённый мастер-договор, запускает планирование изменений
  и применение плана к DOCX;
- показывает последнюю активность GigaCode с временем, моделью и ролью агента;
- показывает текущий раунд и пять параллельных reviewer-ролей;
- отображает замечания и решение synthesis-арбитра;
- повторяет review после подтверждённых исправлений.

## 5. Контроль автоматического результата

Дождитесь одного из состояний:

- «Нужна проверка» — кандидат прошёл автоматический контур;
- «Требуется решение» — пайплайн столкнулся с технической невозможностью
  создать безопасные артефакты или повторяющимся набором замечаний;
- «Ошибка» — техническая ошибка GigaCode или локального инструмента.

Неподтверждённые значения сами по себе не переводят запуск в состояние
«Требуется решение»: они остаются пустыми, получают маркер
`[ТРЕБУЕТСЯ ЗАПОЛНЕНИЕ ЧЕЛОВЕКОМ]` и фиксируются для ручной проверки.

При «Требуется решение» и «Ошибка» подтверждение и финализация заблокированы.
Не запускайте процесс заново, пока не сохранена причина остановки.

При состоянии «Нужна проверка»:

1. просмотрите карточки всех рецензентов и решение арбитра;
2. скачайте «Кандидат DOCX»;
3. при необходимости откройте DOCX в Word или конвертируйте его в PDF для просмотра;
4. сопоставьте кандидат с подписанными PDF;
5. проверьте даты, стороны, реквизиты, номера пунктов, таблицы, сноски,
   колонтитулы и нумерацию страниц;
6. найдите все маркеры `[ТРЕБУЕТСЯ ЗАПОЛНЕНИЕ ЧЕЛОВЕКОМ]` и заполните либо
   явно примите каждое отмеченное место;
7. укажите ФИО проверяющего;
8. нажмите «Подтвердить хеши».

Подтверждение фиксирует SHA-256 кандидата и набора замечаний, но не является
электронной подписью.

## 6. Финализация

После подтверждения нажмите «Финализировать», затем
«Скачать финальный DOCX».

Для независимой проверки результата найдите `RUN_ID`, показанный в UI:

```bash
RUN_DIR="/opt/contractility/data/runs/RUN_ID"
npm run target -- status --run "$RUN_DIR"
npm run target -- verify --run "$RUN_DIR"
cat "$RUN_DIR/final/final-manifest.json"
```

`verify` должен вернуть `"ok": true`. SHA-256 в `final-manifest.json` должен
совпадать с итоговым DOCX.

Остановить локальный сервер после завершения можно сочетанием `Ctrl+C`.

## 7. Резервный CLI-сценарий

Если UI недоступен, те же операции остаются доступны через:

```bash
npm run target -- help
```

CLI следует использовать как резервный интерфейс или для диагностики; штатный
приёмочный прогон выполняется через браузер.

## 8. Расширенная диагностика GigaCode

Для нового диагностического запуска временно установите в
`config/target.json`:

```json
"retainAgentTranscripts": true
```

После перезапуска `npm start` сырые потоки появятся в:

```text
data/runs/RUN_ID/transcripts/
```

Наблюдение за стадиями producer в реальном времени:

```bash
tail -f "$RUN_DIR/transcripts/producer-reconstruct.attempt-1.stdout.ndjson"
tail -f "$RUN_DIR/transcripts/producer-plan.attempt-1.stdout.ndjson"
tail -f "$RUN_DIR/transcripts/producer-apply.attempt-1.stdout.ndjson"
```

Текущее состояние каждой сессии находится в `$RUN_DIR/agent-status/`.
`events.ndjson` содержит только значимые аудиторские события, без записи
каждого фрагмента stdout/stderr.

Логи могут содержать сведения из договора. Каждый поток ограничен 2 МБ и
записывается с правами `0600`. После диагностики верните параметр в `false`.

## Критерии успешного полноценного теста

- зафиксирован точный commit SHA чистой рабочей копии;
- `doctor:target --smoke` подтвердил GigaCode CLI и все выбранные модели;
- `npm test` завершился без ошибок;
- browser OCR обработал весь комплект и сформировал formation request;
- UI повторно передал файлы localhost-серверу и подтвердил их SHA-256;
- первый этап создал мастер-договор без драфта и остановился для ручной проверки;
- подтверждённый мастер выгружен и загружен в пустой комплект с сохранением истории;
- второй этап создал план и кандидат без повторной реконструкции;
- `rounds/01/artifacts/reconstruction-scope.json` перечисляет каждое соглашение
  внутри подписанных PDF и содержит обоснованные решения `included`/`excluded`;
- все пять reviewer-ролей создали отчёты хотя бы в одном раунде;
- synthesis-арбитр сформировал `consensus.json`;
- UI дошёл до состояния «Нужна проверка»;
- человек скачал DOCX/PDF, просмотрел кандидат и подтвердил точные хеши;
- UI финализировал документ и позволил скачать итоговый DOCX;
- резервная команда `verify` вернула `"ok": true`;
- финальный DOCX визуально проверен после конвертации в PDF.

## Что сохранить для аудита

Сохраните целиком каталог `$RUN_DIR`, включая:

- `input-manifest.json` и неизменяемые входы;
- `state.json` и `events.ndjson`;
- все каталоги `rounds/`;
- `approval/approval.json`;
- каталог `final/`;
- использованный `config/target.json` без секретов авторизации;
- версии `node` и `gigacode`.
